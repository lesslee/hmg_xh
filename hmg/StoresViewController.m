...
#import "SIX_STORE_COUNT_REPORT.h"
...
@interface StoresViewController ()<DOPDropDownMenuDataSource,DOPDropDownMenuDelegate,CLLRefreshHeadControllerDelegate>
{
    int currentPage;
    int Max_Count;
    BOOL isRefresh;
...
bool storesCanBack=YES;
@implementation StoresViewController
NSInteger lastColumn8 = -1;
NSInteger lastRow8 = -1;
...
- (void)viewDidLoad {
    ...
    tableView1=[[UITableView alloc] initWithFrame:CGRectMake(0, 72,[UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height - 142)];
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [self.navigationItem setTitle:@"门店数量报表"];
...
{
    StoresCell1  * cell = [StoresCell1 cellWithTableView:tableView1];
    
        //    if (!cell)
        //        {
        //        [tableView registerNib:[UINib nibWithNibName:@"StoresCell" bundle:nil] forCellReuseIdentifier:@"StoresCell"];
        //        cell = [tableView dequeueReusableCellWithIdentifier:@"StoresCell"];
        //        }
...
-(void) tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath != nil) {
        ...
        StoresCell1 *tempCell=(StoresCell1*)cell;
        
        sixStoreCountReport *reportModel=(sixStoreCountReport*)[self.Stores objectAtIndex:indexPath.section];
        ...
        tempCell.hjNU.text = reportModel.TOTAL_COUNT;
        
        tempCell.tbNU.text = reportModel.YOY;
    }
}
#pragma 请求服务器结果
    ...
    @try {
        if(self)
            ...
            if ([@"HMG_DEPT_QUERY" isEqualToString:[dic objectForKey:@"name"]])
                ...
                //                }
            /**查询门店数量**/
            if ([@"SIX_STORE_COUNT_REPORT" isEqualToString:[dic objectForKey:@"name"]])
                {
                sixStoreCountReport *tempReport=[[sixStoreCountReport alloc] init];
                if (isRefresh) {
                    [self.Stores removeAllObjects];
                    [self.Stores addObjectsFromArray:[tempReport searchNodeToArray:xml nodeName:@"NewDataSet"]];
                ...
                if (self.Stores.count>0) {
                    sixStoreCountReport *model0=(sixStoreCountReport *)[self.Stores objectAtIndex:0];
...
- (void)menu:(DOPDropDownMenu *)menu didSelectRowAtIndexPath:(DOPIndexPath *)indexPath
{
    ...
    if (indexPath.column==0) {
        ...
        if (![tempArea.DEPT_ID isEqualToString:@"0"]) {
            ...
            if (indexPath.column==lastColumn8) {
                if (indexPath.row==lastRow8) {
...
- (BOOL)hasRefreshFooterView {
    if (self.Stores.count > 0 && self.Stores.count < Max_Count) {
...
- (void)endRefresh {
    ...
    if (common.isConnectionAvailable) {
        ...
        SIX_STORE_COUNT_REPORT *reportParam=[[SIX_STORE_COUNT_REPORT alloc] init];
        ...
        request1=[ServiceHelper commonSharedRequestMethod:@"SIX_STORE_COUNT_REPORT" soapMessage:paramXml];
        [request1 setUserInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"SIX_STORE_COUNT_REPORT",@"name", nil]];
