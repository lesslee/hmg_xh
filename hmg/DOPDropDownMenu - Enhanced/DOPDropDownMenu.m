...
@property (nonatomic, assign) NSInteger lastSelectedMenudIndex;
...
@property (nonatomic, copy) NSMutableArray *indicators;
@property (nonatomic, copy) NSMutableArray *bgLayers;

@end

#define kTableViewCellHeight 50
#define kTableViewHeight 300
#define kButtomImageViewHeight 40
...
- (void)menuTapped:(UITapGestureRecognizer *)paramSender {
    ...
    //---------------纪录列索引
    _lastSelectedMenudIndex=_currentSelectedMenudIndex;
...
- (BOOL )confiMenuWithSelectRow:(NSInteger)row {
    ...
    NSInteger lastRow=_currentSelectedMenudRow;
    ...
    if (_dataSourceFlags.numberOfItemsInRow && [_dataSource menu:self numberOfItemsInRow:_currentSelectedMenudRow column:_currentSelectedMenudIndex]> 0) {
        ...
        //增加三级联动效果代码，当前列之后的列，清空显示
        
        if (!(_currentSelectedMenudIndex==_lastSelectedMenudIndex&&_currentSelectedMenudRow==lastRow)) {
            for (long i=(_currentSelectedMenudIndex+1); i<self.titles.count; i++) {
                CATextLayer *title1 = (CATextLayer *)_titles[i];
                title1.string = [_dataSource menu:self titleForRowAtIndexPath:[DOPIndexPath indexPathWithCol:i row:0]];
            }
        }
        else
        {
...
- (void)confiMenuWithSelectItem:(NSInteger)item {
    ...
    CATextLayer *title = (CATextLayer *)_titles[_currentSelectedMenudIndex];
    for (int i=_currentSelectedMenudIndex+1; i<self.titles.count; i++) {
        CATextLayer *title1 = (CATextLayer *)_titles[i];
        title1.string = [_dataSource menu:self titleForItemsInRowAtIndexPath:[DOPIndexPath indexPathWithCol:i row:0]];
    }
