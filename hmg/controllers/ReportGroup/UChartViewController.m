...
#import "UUChart.h"
#import "HMG_STORE_FLOW_USER.h"
#import "StoreFlowUser.h"
@interface UChartViewController ()<DOPDropDownMenuDataSource,DOPDropDownMenuDelegate,DateDelegate,UUChartDataSource>
{
    float x;
    int countMonth;
    int z;
...
//地区别流向数据集合
@property(nonatomic,strong)NSMutableArray *userFlow;
...
@implementation UChartViewController
UUChart *chartView;
...
- (void)viewDidLoad {
    ...
    self.userFlow = [[NSMutableArray alloc]init];
...
-(void) finishSingleRequestSuccess:(NSData *)xml userInfo:(NSDictionary *)dic
{
    ...
    /**查询人员别流**/
    if ([@"HMG_STORE_FLOW_USER" isEqualToString:[dic objectForKey:@"name"]])
    {
        StoreFlowUser *tempReport=[[StoreFlowUser alloc] init];
        
        [self.userFlow addObjectsFromArray:[tempReport searchNodeToArray:xml nodeName:@"NewDataSet"]];
        z = (int) self.userFlow.count;
        NSLog(@"%d,0000",z);
        
        StoreFlowUser *model1 = (StoreFlowUser *)[self.userFlow objectAtIndex:0];
...
-(void) loadLayoutInfo
{
    HMG_STORE_FLOW_USER *param=[[HMG_STORE_FLOW_USER alloc] init];
    ...
    ASIHTTPRequest *request1=[ServiceHelper commonSharedRequestMethod:@"HMG_STORE_FLOW_USER" soapMessage:paramXml ];
    [request1 setUserInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"HMG_STORE_FLOW_USER",@"name", nil]];
    [self.serviceHelper addRequestQueue:request1];
    [self.serviceHelper startQueue];
}
...
- (NSArray *)UUChart_xLableArray:(UUChart *)chart
{
    ...
    for (StoreFlowUser *model in _userFlow) {
