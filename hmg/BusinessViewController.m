...
@interface BusinessViewController ()<DOPDropDownMenuDataSource,DOPDropDownMenuDelegate,CLLRefreshHeadControllerDelegate>
{
...
bool bussinessCanBack=YES;
UITableView *tableView1;
UIScrollView *scrollView;


@implementation BusinessViewController
NSInteger lastColumn5 = -1;
NSInteger lastRow5 = -1;
...
- (void)viewDidLoad {
    ...
    self.bussiness=[[NSMutableArray alloc] init];
    self.view.backgroundColor = [self colorWithHexString:@"#e9f1f6" alpha:1];

    tableView1=[[UITableView alloc] initWithFrame:CGRectMake(0, 72,[UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height - 100)];
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [self.navigationItem setTitle:@"出差报表"];
...
-(void) finishSingleRequestSuccess:(NSData *)xml userInfo:(NSDictionary *)dic
{
    @try {
        if(self)
            ...
            /**查询门店走访次数**/
            if ([@"SIX_EVECTION_REPORT" isEqualToString:[dic objectForKey:@"name"]])
                {
                sixEvectionReport *tempReport=[[sixEvectionReport alloc] init];
                if (isRefresh) {
                    [self.bussiness removeAllObjects];
                    [self.bussiness addObjectsFromArray:[tempReport searchNodeToArray:xml nodeName:@"NewDataSet"]];
                ...
                if (self.bussiness.count>0) {
                    sixEvectionReport *model0=(sixEvectionReport *)[self.bussiness objectAtIndex:0];
...
- (void)menu:(DOPDropDownMenu *)menu didSelectRowAtIndexPath:(DOPIndexPath *)indexPath
{
    ...
    if (indexPath.column==0) {
        ...
        if (![tempArea.DEPT_ID isEqualToString:@"0"]) {
            ...
            if (indexPath.column==lastColumn5) {
                if (indexPath.row==lastRow5) {
    ...
    lastColumn5=indexPath.column;
    lastRow5=indexPath.row;
...
{
    BusinessCell1 * cell = [BusinessCell1 cellWithTableView:tableView1];
    
//    if (!cell)
//        {
//        [tableView registerNib:[UINib nibWithNibName:@"BusinessCell" bundle:nil] forCellReuseIdentifier:@"BusinessCell"];
//        cell = [tableView dequeueReusableCellWithIdentifier:@"BusinessCell"];
//        }
...
-(void) tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath != nil) {
        ...
        BusinessCell1 *tempCell=(BusinessCell1*)cell;
        
        sixEvectionReport *reportModel=(sixEvectionReport*)[self.bussiness objectAtIndex:indexPath.section];
        
        tempCell.BusinessName.text = reportModel.EMP_NM;
        tempCell.date.text = reportModel.MON;
        tempCell.province.text = reportModel.PROVINCE;
        tempCell.city.text = reportModel.CITY;
        
        tempCell.BusinessNU.text = reportModel.E_COUNT;
        tempCell.liuxiangScore.text = reportModel.COUNT_SALES;
    }
}
    //禁用于开启后退按钮
-(void) setCanBack:(BOOL) value
{
    bussinessCanBack=value;
}

#pragma 刷新控件

- (BOOL)hasRefreshFooterView {
    if (self.bussiness.count > 0 && self.bussiness.count < Max_Count) {
...
- (void)endRefresh {
    ...
    if (common.isConnectionAvailable) {
        ...
        SIX_EVECTION_REPORT *reportParam=[[SIX_EVECTION_REPORT alloc] init];
        ...
        request1=[ServiceHelper commonSharedRequestMethod:@"SIX_EVECTION_REPORT" soapMessage:paramXml];
        [request1 setUserInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"SIX_EVECTION_REPORT",@"name", nil]];
