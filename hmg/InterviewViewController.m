...
@interface InterviewViewController ()<DOPDropDownMenuDataSource,DOPDropDownMenuDelegate,CLLRefreshHeadControllerDelegate>
{
    int currentPage;
    int Max_Count;
    BOOL isRefresh;
    ...
    NSArray *visitkeys;
    NSMutableDictionary *visitSource;
}
...
UIScrollView *scrollView;
 bool visitCanBack=YES;
@implementation InterviewViewController

NSInteger lastColumn3 = -1;
NSInteger lastRow3 = -1;
...
- (void)viewDidLoad {
    ...
    visitkeys = [[NSArray alloc]initWithObjects:@"StoreName", @"StoreID",nil];
    
    visitSource = [[NSMutableDictionary alloc] initWithObjects:@[@"",@""] forKeys:visitkeys];
    ...
    [startDate1 addTarget:self action:@selector(startDateBtnVistit:) forControlEvents:UIControlEventTouchUpInside];
        //startDate1.backgroundColor = [UIColor colorWithRed:75/255.0 green:192/255.0 blue:220/255.0 alpha:1.0];
...
//    self.startDate = startDate1.titleLabel.text;
    ...
    endDate1.layer.cornerRadius = 5;
        //endDate1.backgroundColor = [UIColor colorWithRed:75/255.0 green:192/255.0 blue:220/255.0 alpha:1.0];
    ...
    .heightIs(30);
        ...
        //storeName.backgroundColor = [UIColor colorWithRed:75/255.0 green:192/255.0 blue:220/255.0 alpha:1.0];
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [self.navigationItem setTitle:@"走访报表"];
...
-(void) finishSingleRequestSuccess:(NSData *)xml userInfo:(NSDictionary *)dic
{
    @try {
        if(self)
            ...
            /**查询走访**/
            if ([@"SIX_VISIT_REPORT" isEqualToString:[dic objectForKey:@"name"]])
                {
                sixVisitReport *tempReport=[[sixVisitReport alloc] init];
                if (isRefresh) {
                    [self.Visits removeAllObjects];
                    [self.Visits addObjectsFromArray:[tempReport searchNodeToArray:xml nodeName:@"NewDataSet"]];
                ...
                if (self.Visits.count>0) {
                    sixVisitReport *model0=(sixVisitReport *)[self.Visits objectAtIndex:0];
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView {
    NSLog(@"y--------------------%lu",(unsigned long)self.Visits.count);
...
{
    InterviewCell * cell = [InterviewCell cellWithTableView:tableView1];
//    if (!cell)
//        {
//        [tableView registerNib:[UINib nibWithNibName:@"InterCell" bundle:nil] forCellReuseIdentifier:@"InterCell"];
//        cell = [tableView dequeueReusableCellWithIdentifier:@"InterCell"];
//        }
...
-(void) tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath != nil) {
        ...
        InterviewCell *tempCell=(InterviewCell*)cell;
        
        sixVisitReport *visitModel=(sixVisitReport*)[self.Visits objectAtIndex:indexPath.section];
        
        NSLog(@"%ld,%@,%lu",(long)indexPath.section,visitModel.EMP_NM,(unsigned long)self.Visits.count);
        tempCell.InterName.text = visitModel.EMP_NM;
        tempCell.InterCity.text = visitModel.DEPT_NM;
//        NSString *storeCN = visitModel.STORE_CNT;
//        tempCell.InterStoresNU.text = [NSString stringWithFormat:@"门店拜访数量: %@",storeCN];
        tempCell.InterStoresNU.text = visitModel.STORE_CNT;
        
//        NSString *interNU = visitModel.REPORT_CNT;
//        tempCell.InterNU.text = [NSString stringWithFormat:@"拜访次数: %@",interNU];
         tempCell.InterNU.text = visitModel.REPORT_CNT;
    }
}
...
- (BOOL)hasRefreshFooterView {
    if (self.Visits.count > 0 && self.Visits.count < Max_Count) {
